...
export default class Ready extends Event<Events.ClientReady> {
    ...
    private async registerPrivateCommands(rest: REST, clientId: string) {
        const privateCommands = (await rest.put(
            Routes.applicationGuildCommands(clientId, process.env.GUILD_ID),
            {
                body: this.GetJson(
                    this.client.commands.filter(
                        (command) => command.privateOnly
                    )
                )
            }
        )) as RESTPostAPIApplicationCommandsResult[]
        consola[privateCommands.length > 0 ? 'success' : 'info'](
            `${privateCommands.length > 0 ? `Successfully loaded ${privateCommands.length} private commands (/)` : 'No private commands found'}`
        )
    }

    private async registerPublicCommands(rest: REST, clientId: string) {
        const publicCommands = (await rest.put(
        ...
        consola[publicCommands.length > 0 ? 'success' : 'info'](
            `${publicCommands.length > 0 ? `Successfully loaded ${publicCommands.length} public commands (/)` : 'No public commands found'}`
        )
    }
