...
export default class CommandHandler extends Event<typeof Events.InteractionCreate> {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (!command) return await interaction.reply({ content: "This command does not exist!\n-# join our support server for more info", ephemeral: true }) && this.client.commands.delete(interaction.commandName);

        if(command.privateOnly && !process.env.PRIVATE_COMMAND_USER_IDS.includes(interaction.user.id)) return interaction.reply({
            embeds: [
                new EmbedBuilder()
                .setColor(this.client.config.colors.error)
                .setTitle("❌ Unable to execute this command :(")
                .setDescription('This command can only be run by developers\n-# Do you think this is a mistake? Please report it to us by joining our support server.')
            ],
            ephemeral: true
        })
